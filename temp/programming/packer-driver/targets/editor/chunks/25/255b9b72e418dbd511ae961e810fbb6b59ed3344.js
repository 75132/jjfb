      ...
      _export("MiniGame1", MiniGame1 = (_dec = ccclass('MiniGame1'), _dec2 = property({
        ...
        openResultPanel() {
          ...
          if (this.resultTipLabel) {
            ...
            let tip = (_RESULT_TIP$code = RESULT_TIP[code]) != null ? _RESULT_TIP$code : RESULT_TIP[5];
            const winName = ((d == null ? void 0 : d.winner_display_name) || '').trim(); // 已开奖且 tip 未强调「谁中了」时，补一行头奖公示（避免与中奖名单区信息脱节）

            if (winName && (d != null && d.draw_finished || d != null && d.after_draw_time) && code !== 4 && code !== 1) {
              tip += `\n\n本期头奖：${winName}`;
            }
