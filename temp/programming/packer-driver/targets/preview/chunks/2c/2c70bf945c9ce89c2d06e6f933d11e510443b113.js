    ...
    execute: function () {
      ...
      _export("ChatRoomFull", ChatRoomFull = (_dec = ccclass('ChatRoomFull'), _dec2 = property({
        ...
        ensureLayout() {
          ...
          var ut = this.content.getComponent(UITransform);
        ...
        onChatMessage(data) {
          ...
          var text = String(data.text || ''); // 玩家消息格式：[世]名：消息
        ...
        onAnnouncement(data) {
          ...
          var text = String(data.text || ''); // 系统消息格式：[世]系统：消息
        ...
        renderCombinedHistory() {
          ...
          if (this.content) {
            // 删除所有子节点，但保留模板节点
            var children = this.content.children.slice();
          ...
          for (var _i = merged.length - 1; _i >= 0; _i--) {
            this.addMessage(merged[_i].line, false);
          }
        ...
        canSend() {
          ...
          var token = (_this$ws$getToken = (_this$ws = this.ws).getToken) == null ? void 0 : _this$ws$getToken.call(_this$ws);
        ...
        updateSendButtonState() {
          ...
          var btn = this.sendButton.getComponent(Button);
        ...
        addMessage(text, scrollTop) {
          ...
          var wasActive = this.messageTemplate.active;
          ...
          var templateTextNode = this.messageTemplate.getChildByName('Text') || this.messageTemplate.getChildByName('text') || this.messageTemplate.getChildByName('T Text');

          if (!templateTextNode) {
            var children = this.messageTemplate.children;
          ...
          var templateTextUT = templateTextNode.getComponent(UITransform);
          ...
          var originalTextAnchorX = templateTextUT.anchorX;
          var originalTextAnchorY = templateTextUT.anchorY;
          var originalTextPos = templateTextNode.position.clone();
          var originalTextWidth = templateTextUT.width;
          var originalTextHeight = templateTextUT.height; // 克隆 Message 节点

          var messageNode = instantiate(this.messageTemplate); // 恢复模板节点的原始状态（在克隆后立即恢复）
          ...
          var textNode = messageNode.getChildByName('Text') || messageNode.getChildByName('text') || messageNode.getChildByName('T Text'); // 如果还没找到，尝试查找所有子节点中带有 Label 组件的节点

          if (!textNode) {
            var _children = messageNode.children;

            for (var _i2 = 0; _i2 < _children.length; _i2++) {
              if (_children[_i2].getComponent(Label)) {
                textNode = _children[_i2];
          ...
          var textUT = textNode.getComponent(UITransform);
          ...
          var isSystemMessage = text.startsWith('[世]系统：'); // 设置文本颜色：系统消息红色，玩家消息白色
          ...
          var layout = this.content.getComponent(Layout);
          var layoutWasEnabled = layout ? layout.enabled : false;
          ...
          if (textUT) {
            ...
            var messageUT = messageNode.getComponent(UITransform);

            if (messageUT) {
              // Message 节点的锚点应该与模板一致
              var templateMessageUT = this.messageTemplate.getComponent(UITransform);
          ...
          this.scheduleOnce(() => {
            ...
            var labelUT = textNode.getComponent(UITransform);
            var messageUT = messageNode.getComponent(UITransform);

            if (labelUT && messageUT) {
              // 关键修复：根据最佳实践，在调整高度前暂时禁用 Layout
              var _layoutWasEnabled = layout ? layout.enabled : false;
              ...
              var textLocalPos = originalTextPos.clone();
              var textAnchorX = originalTextAnchorX;
              var textAnchorY = originalTextAnchorY; // 获取文本的实际高度

              var textHeight = labelUT.height; // Message 高度 = Text 高度 

              var messageHeight = textHeight; // 调整Message高度（这可能会影响子节点的位置计算）
          ...
          var allChildren = this.content.children;
          var messageCount = 0;

          for (var _i3 = 0; _i3 < allChildren.length; _i3++) {
            if (allChildren[_i3] !== this.messageTemplate) {
              messageCount++;
            }
          }
          ...
          if (messageCount > this.maxItems) {
            // 找到最后一个非模板节点
            for (var _i4 = allChildren.length - 1; _i4 >= 0; _i4--) {
              var child = allChildren[_i4];
        ...
        updateLengthDisplay() {
          ...
          var currentLength = this.inputBox ? this.inputBox.string.length : 0; // 查找Lenth节点下的Label组件

          var label = this.lenthNode.getComponent(Label);

          if (!label) {
            // 如果没有直接挂载Label，尝试查找子节点
            var textNode = this.lenthNode.getChildByName('Text') || this.lenthNode.getChildByName('text');
        ...
        onClickSend() {
          ...
          var text = this.inputBox ? String(this.inputBox.string || '').trim() : '';

          if (!text) {
            return;
          }

          var cid = ((_this$ws$getCharacter = (_this$ws3 = this.ws).getCharacterId) == null ? void 0 : _this$ws$getCharacter.call(_this$ws3)) || undefined; // 优化：使用request方法，自动生成request_id并匹配响应
          ...
          this.ws.request('post_chat', {
            ...
            if (!response || !response.success) {
              var errorMsg = (response == null ? void 0 : response.error) || (response == null ? void 0 : response.message) || '发送失败';
        ...
        setMessageImageVisible(messageNode, visible) {
          ...
          var sprite = messageNode.getComponent(Sprite);
        ...
        setupMessageClickHandler(messageNode) {
          ...
          var button = messageNode.getComponent(Button);
...
//# sourceMappingURL=2c70bf945c9ce89c2d06e6f933d11e510443b113.js.map