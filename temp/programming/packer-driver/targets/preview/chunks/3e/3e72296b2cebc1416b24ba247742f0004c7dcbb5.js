System.register(["cc"], function (_export, _context) {
  ...
  function isPseudoBattleResultScript(script) {
    ...
    var opts = (_script$options = script == null ? void 0 : script.options) != null ? _script$options : [];
    if (opts.length === 0) return false;
    var hasPseudoWin = opts.some(o => {
      var _o$text;

      return PSEUDO_WIN_TEXT.test(String((_o$text = o.text) != null ? _o$text : '').trim());
    });
    var hasPseudoLose = opts.some(o => {
      var _o$text2;
  ...
  function sanitizeBattlePseudoChoicesInRuntime(map) {
    ...
    var scripts = (_map$client$choiceScr = (_map$client = map.client) == null ? void 0 : _map$client.choiceScripts) != null ? _map$client$choiceScr : {};
    var removedScriptIds = new Set();
    var battleEventsFixed = 0;

    for (var npc of (_map$npcs = map.npcs) != null ? _map$npcs : []) {
      var _map$npcs;

      for (var ev of (_npc$events = npc.events) != null ? _npc$events : []) {
        var _npc$events, _ev$client, _ev$server;

        if (ev.eventType !== 'battle') continue;
        var sid = (_ev$client = ev.client) == null ? void 0 : _ev$client.choiceScriptId;
        if (!sid) continue;
        var script = scripts[sid];
    ...
    for (var _sid of removedScriptIds) {
      delete scripts[_sid];
    }
