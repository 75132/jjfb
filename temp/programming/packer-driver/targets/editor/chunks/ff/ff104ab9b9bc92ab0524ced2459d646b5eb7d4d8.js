      ...
      _export("MapNpcInteract", MapNpcInteract = (_dec = ccclass('MapNpcInteract'), _dec2 = executionOrder(-50), _dec3 = property(JsonAsset), _dec4 = property(Node), _dec5 = property({
        ...
        constructor(...args) {
          ...
          this._onKeyDown = e => {
            ...
            const existing = this.uiRoot.getComponent(_crd && StoryDialoguePlayer === void 0 ? (_reportPossibleCrUseOfStoryDialoguePlayer({
              error: Error()
            ...
            const ev = this._pickInteractEvent();
            ...
            const player = this._ensureStoryPlayer();

            if (!player) return;
            const client = (_ev$client = ev.client) != null ? _ev$client : {};
        ...
        onLoad() {
          if (!this.letBattleTriggerHandleCombat) {
            const battle = this.getComponent(_crd && BattleTriggerOnContact === void 0 ? (_reportPossibleCrUseOfBattleTriggerOnContact({
              error: Error()
        ...
        _parseMap() {
          ...
          const raw = (_this$mapConfig2 = this.mapConfig) == null ? void 0 : _this$mapConfig2.json;
          ...
          const npcs = (_raw$npcs = raw.npcs) != null ? _raw$npcs : [];
          const npc = npcs.find(n => n.npcUid === this.npcUid);
          ...
          if (Number.isFinite(maxD) && maxD > 0) {
            const before = this.interactDistanceFallbackPx;
        ...
        _markEventDone(ev) {
          const id = this._stableEventId(ev);
        ...
        _pickInteractEvent() {
          const sorted = [...this._npcEvents].sort((a, b) => {
          ...
          for (const ev of sorted) {
            ...
            const reqs = (_ev$server = ev.server) == null ? void 0 : _ev$server.requirements;
        ...
        _ensureStoryPlayer() {
          ...
          let p = this.uiRoot.getComponent(_crd && StoryDialoguePlayer === void 0 ? (_reportPossibleCrUseOfStoryDialoguePlayer({
            error: Error()
          }), StoryDialoguePlayer) : StoryDialoguePlayer);
        ...
        _resolveLocalPlayerOnce() {
          const now = Date.now();
          const resolved = Boolean(this._playerMove && this._playerCollider); // 仅在已解析到玩家时做 2s 节流；首帧/预制体晚到时必须每帧重试，否则会永久拿不到碰撞体
          ...
          try {
            ...
            const pNode = (_this$_playerMove = this._playerMove) == null ? void 0 : _this$_playerMove.node;
        ...
        _computeAabbOverlap() {
          const trig = this._triggerBox;
          if (!trig || !this._playerCollider) return false;
          const a = trig.worldAABB;
          const b = this._playerCollider.worldAABB;
        ...
        _pollTouchOverlap() {
          ...
          const prevTouching = this._playerTouching;
          ...
          const aabbOverlap = this._computeAabbOverlap();
          ...
          if (prevTouching !== hit) {
            ...
            if (hit) {
              const p = this._ensureStoryPlayer();
          ...
          if (this.debugLog && !hit && (_this$_playerMove3 = this._playerMove) != null && _this$_playerMove3.node) {
            const near = dist < R * 2.2;
            const now = Date.now();

            if (near && now - this._lastRangeDiagLogAt > 3000) {
              this._lastRangeDiagLogAt = now;
              const npcWp = this.node.worldPosition;
              const plWp = this._playerMove.node.worldPosition;
              (_crd && storyLog === void 0 ? (_reportPossibleCrUseOfstoryLog({
                error: Error()
