System.register(["cc"], function (_export, _context) {
  ...
  function logicalToTileIndices(lx, ly, tileSize) {
    if (tileSize === void 0) {
      tileSize = TILE_CELL;
    }

    var tx = Math.round((lx - tileSize / 2) / tileSize);
    var ty = Math.round((tileSize / 2 - ly) / tileSize);
    return {
      tx,
      ty
    };
  ...
  function tileIndicesToLogical(tx, ty, tileSize) {
    if (tileSize === void 0) {
      tileSize = TILE_CELL;
    }
  ...
  function logicalYToImagePy(ly, mapH, tileSize) {
    if (tileSize === void 0) {
      tileSize = TILE_CELL;
    }

    if (mapH <= tileSize / 2) return tileSize / 2;
    var tyMax = Math.max(0, Math.floor((mapH - tileSize / 2 - 1) / tileSize));
    var ty = Math.round((tileSize / 2 - ly) / tileSize);
    var cty = Math.max(0, Math.min(tyMax, ty));
    return cty * tileSize + tileSize / 2;
  }
  /** 逻辑格心 → 地图父节点本地坐标（内容区左上为 minX/maxY，与 Juben MapEditorView 一致） */


  function logicalToParentLocal(lx, ly, bounds, tileSize) {
    if (tileSize === void 0) {
      tileSize = TILE_CELL;
    }

    var mapH = bounds.maxY - bounds.minY;
    var imagePy = logicalYToImagePy(ly, mapH, tileSize);
    return {
      x: bounds.minX + lx,
  ...
  function mapContentBoundsInParentSpace(mapPos, mapUt) {
    var w = mapUt.width;
    var h = mapUt.height;
    var left = mapPos.x - mapUt.anchorX * w;
    var right = left + w;
    var bottom = mapPos.y - mapUt.anchorY * h;
    var top = bottom + h;
    return {
      minX: left,
      maxX: right,
