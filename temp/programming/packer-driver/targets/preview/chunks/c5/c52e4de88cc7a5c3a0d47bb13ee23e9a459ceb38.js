System.register(["cc"], function (_export, _context) {
  ...
  function eventHasEffect(ev, action, taskId) {
    ...
    var effects = (_ev$server$effects = (_ev$server = ev.server) == null ? void 0 : _ev$server.effects) != null ? _ev$server$effects : [];
  ...
  function isCurrentSegmentAccepted(sorted, npcUid, ctx) {
    var segmentAcceptEv = null;

    for (var _ev of sorted) {
      if (eventIsTaskTurnIn(_ev) && ctx.isStepComplete(npcUid, _ev)) {
        segmentAcceptEv = null;
        continue;
      }

      if (!ctx.isStepComplete(npcUid, _ev)) {
        if (eventIsTaskAccept(_ev)) return false;
        return segmentAcceptEv != null;
      }

      if (eventIsTaskAccept(_ev)) {
        segmentAcceptEv = _ev;
      }
    }
  ...
  function findCurrentSegmentTurnIn(sorted, npcUid, ctx) {
    ...
    for (var _ev2 of sorted) {
      if (eventIsTaskTurnIn(_ev2) && ctx.isStepComplete(npcUid, _ev2)) {
        var _ev2$order;

        segmentStartOrder = ((_ev2$order = _ev2.order) != null ? _ev2$order : 0) + 1;
      }
    }

    for (var _ev3 of sorted) {
      var _ev3$order;

      if (((_ev3$order = _ev3.order) != null ? _ev3$order : 0) < segmentStartOrder) continue;
      if (!eventIsTaskTurnIn(_ev3)) continue;
      if (!ctx.isStepComplete(npcUid, _ev3)) return _ev3;
    }
  ...
  function hasPendingTurnInRequirements(turnInEv, ctx) {
    ...
    var reqs = (_turnInEv$server = turnInEv.server) == null ? void 0 : _turnInEv$server.requirements;
  ...
  function hasIncompleteBattleBeforeTurnIn(sorted, npcUid, turnInEv, ctx) {
    ...
    var turnOrder = (_turnInEv$order = turnInEv.order) != null ? _turnInEv$order : Number.MAX_SAFE_INTEGER;

    for (var _ev4 of sorted) {
      var _ev4$order;

      if (((_ev4$order = _ev4.order) != null ? _ev4$order : 0) >= turnOrder) break;
      if (_ev4.eventType !== 'battle') continue;
      if (!ctx.isStepComplete(npcUid, _ev4)) return true;
    }
  ...
  function isTurnInReady(sorted, npcUid, turnInEv, ctx) {
    ...
    var turnOrder = (_turnInEv$order2 = turnInEv.order) != null ? _turnInEv$order2 : Number.MAX_SAFE_INTEGER;

    for (var _ev5 of sorted) {
      var _ev5$order;

      if (((_ev5$order = _ev5.order) != null ? _ev5$order : 0) >= turnOrder) continue;
      if (eventIsTaskTurnIn(_ev5)) continue;
      if (!ctx.isStepComplete(npcUid, _ev5)) return false;
    }
  ...
  function extractPrimaryTaskId(events) {
    for (var _ev6 of events) {
      for (var eff of (_ev6$server$effects = (_ev6$server = _ev6.server) == null ? void 0 : _ev6$server.effects) != null ? _ev6$server$effects : []) {
        var _ev6$server$effects, _ev6$server, _eff$action2;
  ...
  function resolveNpcTaskIndicatorKind(npcUid, events, ctx) {
    ...
    for (var _ev7 of sorted) {
      if (!ctx.isStepComplete(npcUid, _ev7)) {
        firstIncomplete = _ev7;
        break;
      }
    }

    if (!firstIncomplete) return null;
    var segmentAccepted = isCurrentSegmentAccepted(sorted, npcUid, ctx);
    var reqs = (_firstIncomplete$serv = firstIncomplete.server) == null ? void 0 : _firstIncomplete$serv.requirements;
    ...
    var turnInEv = findCurrentSegmentTurnIn(sorted, npcUid, ctx);
