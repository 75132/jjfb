...
export class GameConfig {
    ...
    static readonly AUTH_INACTIVITY_RELOGIN_MS = 1000 * 60 * 10; // 10m
    
    // 监控配置（优化：减少检查频率，提高性能）
    static readonly TOKEN_CHECK_INTERVAL = 10000; // 增加到10秒，减少检查频率
    static readonly TOKEN_CHECK_INTERVAL_FAST = 5000; // 增加到5秒
    static readonly TOKEN_CHECK_INTERVAL_GAME = 15000; // 游戏场景中使用15秒，进一步减少性能消耗
    ...
    static setWsUrl(url: string): void {
        try {
            const win: any = (typeof window !== 'undefined') ? window : {};
            win.__WS_URL__ = url;
            if (win.localStorage) { win.localStorage.setItem('WS_URL', url); }
        } catch {}
    }
}
