    ...
    execute: function () {
      ...
      _export("MiniGame2", MiniGame2 = (_dec = ccclass('MiniGame2'), _dec2 = property({
            ...
            this.ws.request('minigame2_bet', {
              ...
              if (resp != null && resp.success && resp != null && resp.data) {
                ...
                if (respSelected && respAmt) {
                  var catName = this.rowDisplayName(String(respSelected));
        ...
        updateOptionVisual() {
          ...
          var canBetBase = p ? !p.round_drawn && ((_p$seconds_until_clos = p.seconds_until_close) != null ? _p$seconds_until_clos : 0) > 0 : false;
          var winnerKey = p != null && p.round_drawn ? String((_p$winner_key = p.winner_key) != null ? _p$winner_key : '').trim() : '';

          for (var item of this.optionItems) {
            if (!item.sprite || !item.normalSprite || !item.pressedSprite) continue;
            var isWinner = !!winnerKey && item.key === winnerKey;
            var isSelected = this.selectedKey === item.key && canBetBase;
            var usePressed = isWinner || isSelected;
        ...
        resolveCategoryRows(p) {
          if (Array.isArray(p.categories) && p.categories.length > 0) {
            ...
            for (var c of p.categories) {
              ...
              var key = String((_key = c == null ? void 0 : c.key) != null ? _key : '').trim();
              if (!key) continue;
              var name = String((_name = c == null ? void 0 : c.name) != null ? _name : key).trim() || key;
              var mult = Math.floor(Number(c == null ? void 0 : c.multiplier) || 0);
              out.push({
                key,
                name,
        ...
        rebuildOptionItems(rows, token) {
          var _this2 = this;

          return _asyncToGenerator(function* () {
            if (!_this2.optionContent) return;
            var prefab = yield _this2.ensureOptionItemPrefab();
            if (token !== _this2._rebuildToken) return;
            _this2._displayRows = rows.slice();

            try {
              var _this2$_lastPayload;
              ...
              var _loop = function* _loop() {
                if (prefab) {
                  ...
                  var keyRef = cat.key;
                  var nameRef = cat.name;
                  btnComp.node.on(Button.EventType.CLICK, () => {
                    var _this2$_lastPayload2, _this2$_lastPayload$s, _this2$_lastPayload3;

                    if ((_this2$_lastPayload2 = _this2._lastPayload) != null && _this2$_lastPayload2.round_drawn) return;
                    if (((_this2$_lastPayload$s = (_this2$_lastPayload3 = _this2._lastPayload) == null ? void 0 : _this2$_lastPayload3.seconds_until_close) != null ? _this2$_lastPayload$s : 0) <= 0) return;
                    _this2.selectedKey = keyRef;
                    if (_this2.currentSelectionLabel) _this2.currentSelectionLabel.string = nameRef;
                  ...
                  var _keyRef = cat.key;
                  var _nameRef = cat.name;
                  btn.node.on(Button.EventType.CLICK, () => {
                    var _this2$_lastPayload4, _this2$_lastPayload$s2, _this2$_lastPayload5;

                    if ((_this2$_lastPayload4 = _this2._lastPayload) != null && _this2$_lastPayload4.round_drawn) return;
                    if (((_this2$_lastPayload$s2 = (_this2$_lastPayload5 = _this2._lastPayload) == null ? void 0 : _this2$_lastPayload5.seconds_until_close) != null ? _this2$_lastPayload$s2 : 0) <= 0) return;
                    _this2.selectedKey = _keyRef;
                    if (_this2.currentSelectionLabel) _this2.currentSelectionLabel.string = _nameRef;
              ...
              var validKeys = new Set(rows.map(r => r.key));

              if (_this2.selectedKey && !validKeys.has(_this2.selectedKey)) {
                var _rows$0$key, _rows$;

                _this2.selectedKey = (_rows$0$key = (_rows$ = rows[0]) == null ? void 0 : _rows$.key) != null ? _rows$0$key : null;
              }

              if (!_this2.selectedKey && rows.length > 0) {
                _this2.selectedKey = rows[0].key;
              }

              if (_this2.currentSelectionLabel && _this2.selectedKey && !((_this2$_lastPayload = _this2._lastPayload) != null && _this2$_lastPayload.round_drawn)) {
                _this2.currentSelectionLabel.string = _this2.rowDisplayName(_this2.selectedKey);
              }
            ...
            if (_this2._lastPayload) {
              _this2.applyInteractableFromPayload(_this2._lastPayload);
            }
        ...
        requestInvestmentReturnHistory() {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this4.ws.request('minigame2_return_history_sync', {}, resp => {
              ...
              _this4.resetHistoryPanelAfterError();
        ...
        applyInteractableFromPayload(p) {
          ...
          var canBetBase = !p.round_drawn && ((_p$seconds_until_clos2 = p.seconds_until_close) != null ? _p$seconds_until_clos2 : 0) > 0;
          var canBet = canBetBase && !this.investmentReturnHistoryOpen;
        ...
        applyPayload(p) {
          ...
          this._lastPayload = p;
          var rows = this.resolveCategoryRows(p);
          ...
          if (this.myBetsLabel) {
            ...
            var total = Math.floor(Number((_p$my_bet_total = p.my_bet_total) != null ? _p$my_bet_total : 0));
            ...
            if (bets.length === 0) {
              ...
              var _loop2 = function _loop2() {
                ...
                var name = (_rows$find$name = (_rows$find = rows.find(r => r.key === key)) == null ? void 0 : _rows$find.name) != null ? _rows$find$name : key;
          ...
          if (p.server_time) {
            ...
            if (parsed != null) {
              ...
              var sec = p.round_drawn ? 0 : (_p$seconds_until_clos3 = p.seconds_until_close) != null ? _p$seconds_until_clos3 : 0;
          ...
          if (p.round_drawn && this.currentSelectionLabel) {
            ...
            var wk = String((_p$winner_key2 = p.winner_key) != null ? _p$winner_key2 : '').trim();
            var wn = wk ? this.rowDisplayName(wk) : '';
            if (wn) this.currentSelectionLabel.string = "\u672C\u671F\u5F00\u5956\uFF1A" + wn;
          }

          var sig = this.categoryStructureSig(rows);
          var needRebuild = sig !== this._optionCategorySig || this.optionItems.length !== rows.length;
        ...
        onConfirmInvestmentClick() {
          ...
          var catName = this.rowDisplayName(this.selectedKey);
