...
export class RequestRetryManager {
     ...
     * 使用计数器+随机数确保唯一性
     */
    static generateRequestId(route: string, data: any): string {
        // 使用计数器确保唯一性（即使在同一毫秒内）
        this.requestCounter = (this.requestCounter + 1) % 1000000;
        
        // 使用路由、数据、时间戳、计数器和随机数生成唯一ID
     ...
     * 网络错误可重试，业务错误不重试
     */
    static shouldRetry(response: any, config: RetryConfig = {}): boolean {
     ...
     * 计算退避延迟时间
     */
    static calculateBackoffDelay(attempt: number, config: RetryConfig = {}): number {
     ...
     */
    static isIdempotentRoute(route: string): boolean {
        // 查询操作通常是幂等的
        if (route.startsWith('get_') || 
            route.startsWith('search_') ||
            route === 'get_player' ||
            route === 'get_character_info' ||
            route === 'get_all_characters' ||
            route === 'get_robot_pets' ||
            route === 'get_robot_pet_info' ||
            route === 'bag_get' ||
            route === 'get_chat_history' ||
            route === 'get_announcements_history' ||
        ...
        // 修改操作通常不是幂等的
        if (route === 'upgrade_robot' ||
            route === 'upgrade_all_robots' ||
            route === 'bag_use_item' ||
            route === 'bag_discard_item' ||
            route === 'add_exp' ||
            route === 'add' ||
            route === 'create_character' ||
            route === 'delete_character' ||
            route === 'add_friend' ||
            route === 'approve_friend' ||
            route === 'reject_friend' ||
            route === 'delete_friend' ||
            route === 'post_chat' ||
            route === 'post_announcement' ||
