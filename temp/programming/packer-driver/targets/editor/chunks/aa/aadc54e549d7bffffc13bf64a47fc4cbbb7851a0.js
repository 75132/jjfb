    ...
    execute: function () {
      ...
      _export("PlayerGridMove", PlayerGridMove = (_dec = ccclass('PlayerGridMove'), _dec2 = property({
        ...
        start() {
          ...
          const hasPlayerStateSync = this.deferInitialPlaceToServerRestore && !!((_anyNode$getComponent = anyNode == null || anyNode.getComponent == null ? void 0 : anyNode.getComponent('PlayerStateSync')) != null ? _anyNode$getComponent : null);
        ...
        update(dt) {
          const dashing = this.dashLikeMV && (this._heldCodes.has(KeyCode.SHIFT_LEFT) || this._heldCodes.has(KeyCode.SHIFT_RIGHT));
        ...
        _tryBeginStep(deltaCol, deltaRow) {
          ...
          const rr = this.useAnchorAsGridOrigin ? Math.floor((originY - cur.y) / CELL) : Math.floor((cur.y - originY) / CELL);
        ...
        placeAtGrid(col, row) {
          ...
          const y = this.useAnchorAsGridOrigin ? m.originY - (r + 0.5) * CELL : m.originY + (r + 0.5) * CELL;
        ...
        setAnimPrefix(prefix, refreshNow = true) {
          const p = (prefix || '').trim();
          if (!p) return false;
        ...
        applyServerSprite(spriteIndex, refreshNow = true) {
          if (this._animRt) {
        ...
        _snapToGridFromCurrentPos() {
          ...
          let rr = this.useAnchorAsGridOrigin ? Math.floor((originY - cur.y) / CELL) : Math.floor((cur.y - originY) / CELL);
          cc = Math.min(cols - 1, Math.max(0, cc));
          rr = Math.min(rows - 1, Math.max(0, rr));
          const cx = originX + (cc + 0.5) * CELL;
          const cy = this.useAnchorAsGridOrigin ? originY - (rr + 0.5) * CELL : originY + (rr + 0.5) * CELL;
        ...
        _mapGridMetrics(map, mapUt) {
          ...
          const cols = this.useAnchorAsGridOrigin ? Math.floor((b.maxX - originX) / CELL) : Math.floor((b.maxX - b.minX) / CELL);
          const rows = this.useAnchorAsGridOrigin ? Math.floor((originY - b.minY) / CELL) : Math.floor((b.maxY - b.minY) / CELL);
          return {
            originX,
            originY,
            cols,
        ...
        _mapBoundsInParentSpace(map, mapUt) {
          ...
          const parentUt = (_map$parent = map.parent) == null ? void 0 : _map$parent.getComponent(UITransform);
        ...
        _refreshLayerNameSets() {
          const toSet = raw => new Set(raw.split(',').map(s => s.trim().toLowerCase()).filter(s => s.length > 0));
        ...
        _hasRequiredPrefixClips(prefix) {
          ...
          const required = [`${prefix}_idle_down`, `${prefix}_idle_left`, `${prefix}_idle_right`, `${prefix}_idle_up`, `${prefix}_walk_down`, `${prefix}_walk_left`, `${prefix}_walk_right`, `${prefix}_walk_up`];
