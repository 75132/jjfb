      ...
      _export("MechAttributeTEST", MechAttributeTEST = (_dec = ccclass('MechAttributeTEST'), _dec2 = property({
        ...
        constructor() {
          ...
          this.onRobotInfo = data => {
            try {
              ...
              var petId = rawPetId !== null && rawPetId !== undefined ? String(rawPetId) : null; // 如果界面已有目标机甲ID，而本次消息没有 petId 或者与当前不一致，则跳过
        ...
        showSelectedRobot(petId) {
          ...
          return _asyncToGenerator(function* () {
            try {
              // 关键修复：设置当前应该显示的机甲ID
              _this7.currentPetId = petId ? String(petId) : null; // 关键修复：先尝试使用缓存数据，解决首次打开不显示的问题

              var cacheManager = (_crd && DataCacheManager === void 0 ? (_reportPossibleCrUseOfDataCacheManager({
                error: Error()
              }), DataCacheManager) : DataCacheManager).getInstance();
              var cachedData = cacheManager.getRobotPetInfoCache(petId); // 关键修复：通知 MechEquipment 组件设置 currentPetId
