System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  function isHiddenByMainlineStep(row, state) {
    ...
    for (var raw of row.events) {
      var ev = raw;

      for (var req of (_ev$server$requiremen = (_ev$server = ev.server) == null ? void 0 : _ev$server.requirements) != null ? _ev$server$requiremen : []) {
        var _ev$server$requiremen, _ev$server, _ref, _r$type;

        var r = req;
        var t = String((_ref = (_r$type = r.type) != null ? _r$type : r.action) != null ? _ref : '');
  ...
  function resolveCurrentMainlineNpcUid(state) {
    for (var _uid of state.storyNpcOrder) {
      if (state.isBattleEnemyNpcUid(_uid)) continue;
      if (state.isNpcHiddenByAppear(_uid, undefined)) continue;
      var _row = {
        npcUid: _uid
      };
      if (isHiddenByMainlineStep(_row, state)) continue;
      if (state.hasActiveInteractEvent(_uid, [])) return _uid;
    }
  ...
  function isStaleMainlineGiver(npcUid, storyNpcOrder, isBattleEnemy, isHiddenByAppear, hasIncomplete, hasInteract) {
    ...
    var myIdx = storyNpcOrder.indexOf(npcUid);
    if (myIdx < 0) return false;

    for (var i = myIdx + 1; i < storyNpcOrder.length; i++) {
      var later = storyNpcOrder[i];
  ...
  function decideNpcVisibility(npcUid, row, events, state, currentMainlineUid, isAncestorOfCurrent) {
    if (state.isBattleEnemyNpcUid(npcUid)) {
      var _show = state.hasActiveInteractEvent(npcUid, events);

      return {
        visible: _show,
        colliderEnabled: _show,
    ...
    if (!state.sequentialReveal) {
      ...
      var active = state.hasActiveInteractEvent(npcUid, events);
      return {
    ...
    if (currentMainlineUid === null) {
      var _active = state.hasActiveInteractEvent(npcUid, events);
    ...
    var isCurrent = npcUid === currentMainlineUid;
    var show = isCurrent || isAncestorOfCurrent;
    return {
      visible: show,
  ...
  function npcAppearRequirementsMet(row, ctx) {
    var appear = row == null ? void 0 : row.appear;
