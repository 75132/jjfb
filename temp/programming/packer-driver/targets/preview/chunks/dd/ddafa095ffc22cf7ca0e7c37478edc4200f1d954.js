    ...
    execute: function () {
      ...
      _export("RobotEvolutionEffect", RobotEvolutionEffect = (_dec = ccclass('RobotEvolutionEffect'), _dec2 = property({
        ...
        static getInstance() {
          ...
          var comp = panel.getComponent(RobotEvolutionEffect);
        ...
        init() {
          ...
          var root = this.spriteNode || this.rootNode || this.node;
        ...
        playEvolution(oldAniId, newAniId) {
          ...
          var oldFrame = this.getFrameImmediate(this.currentOldFrameName);
          var newFrame = this.getFrameImmediate(this.currentNewFrameName); // 如果有旧形态的帧，立即显示
        ...
        startSwitchAnimation() {
          ...
          var doSwitch = () => {
            ...
            var now = Date.now();
            var elapsed = (now - this.startTime) / 1000; // 已过时间（秒）
            // 获取当前帧

            var oldF = this.getFrameImmediate(this.currentOldFrameName);
            var newF = this.getFrameImmediate(this.currentNewFrameName); // 如果两个帧都加载了，进行切换
            ...
            if (oldF && newF) {
              ...
              var progress = Math.min(elapsed / this.switchTotal, 1);
              var easeOutProgress = 1 - (1 - progress) * (1 - progress); // easeOutQuad

              var currentInterval = this.minInterval + (this.maxInterval - this.minInterval) * easeOutProgress; // 取消之前的调度，用新间隔重新调度
        ...
        loadFrameAsync(frameName) {
          ...
          if (this.frameCache.has(frameName)) {
            ...
            if (this.isPlaying && this.primarySprite) {
              var oldF = this.getFrameImmediate(this.currentOldFrameName);
              var newF = this.getFrameImmediate(this.currentNewFrameName);
          ...
          (_crd && ResourceManager === void 0 ? (_reportPossibleCrUseOfResourceManager({
            ...
            if (this.isPlaying && this.primarySprite) {
              ...
              var _oldF = this.getFrameImmediate(this.currentOldFrameName);

              var _newF = this.getFrameImmediate(this.currentNewFrameName);
...
//# sourceMappingURL=ddafa095ffc22cf7ca0e7c37478edc4200f1d954.js.map