...
@ccclass('CharacterProfile')
export class CharacterProfile extends Component {
    @property(Node) nameNode: Node = null!;
    @property(Node) levelNode: Node = null!;
    @property(Node) idNode: Node = null!;
    @property(Node) classNode: Node = null!;
    @property(Node) coinsNode: Node = null!;
    @property(Node) energyBlocksNode: Node = null!; // 能量块（第二货币），可选绑定
    @property(Node) robotCountNode: Node = null!;  // 机甲数量节点
    @property(Label) expLabel: Label = null!;
    @property(Node) expPoint: Node = null!;
    @property(Sprite) spriteComp: Sprite = null!;
    @property([SpriteFrame]) spriteFrames: SpriteFrame[] = [];
    @property(Button) backButton: Button = null!;
    @property(Label) onlineStatusLabel: Label = null!;  // 在线状态Label（可选，如果UI中有则绑定，直接使用Label显示）
    
    // ====== 操作按钮相关（可选，如果UI中没有则可以为null） ======
    @property(Node) actionButtonsNode: Node = null!;  // 操作按钮容器节点
    @property(Button) addFriendButton: Button = null!;  // 添加好友按钮
    @property(Button) deleteFriendButton: Button = null!;  // 删除好友按钮
    @property(Button) sendMessageButton: Button = null!;  // 发送消息按钮

    private ws: WebSocketManager = null!;
    private readonly barMax = 338;
    private viewMode: 'self' | 'friend' = 'self';  // 查看模式：自己或好友
    
    // ====== 状态管理 ======
    private currentConfig: ProfileConfig | null = null;  // 当前配置
    private isRequesting: boolean = false;  // 是否正在请求数据
    private isFriend: boolean = false;  // 当前查看的角色是否是好友
    private targetFriendId: string = '';  // 目标好友ID（用于添加/删除好友操作）
    private currentRequestId: number = 0;  // 当前请求ID，用于验证响应是否属于自己
    private isShowMethodCalled: boolean = false;  // 标记是否是通过 show() 方法打开的（用于区分直接激活）

    onLoad() {
        this.ws = WebSocketManager.getInstance();
        // ✅ 修复：服务器使用 send_success_response 发送的是 'player_info_response' 事件
        ...
        if (this.backButton && this.backButton.node) {
            this.backButton.node.on(Button.EventType.CLICK, () => { this.node.active = false; }, this);
        }
    ...
    public show(config: ProfileConfig): void {
        ...
        // 发起数据请求（这会设置 isRequesting=true 并生成新的 request_id）
        this.requestData();
    }
     ...
     * @deprecated 请使用 show()
     */
    public showFriendInfo(characterId?: string, friendId?: string): void {
        this.show({
            entryType: ProfileEntryType.FRIEND_LIST, // 默认假设来自好友列表，或者是搜索结果
            characterId,
            friendId
        });
    }
    ...
    private onPlayerInfo = (data: any) => {
        ...
        // 如果是标准响应格式（使用 send_success_response），数据在 data.data 中
        ...
        this.targetFriendId = responseData.friend_id ?? '';
        
        // ✅ 提取所有字段，添加详细日志
        // 注意：字段名必须与服务器返回的完全一致（区分大小写）
        const name = responseData.role_name ?? '';
        const level = Number(responseData.level ?? 0);
        const fid = responseData.friend_id ?? '';
        const cls = Number(responseData.class ?? 0);
        const gold = Number(responseData.gold ?? 0);
        const energyBlocks = getEnergyBlocksFromPayload(responseData);
        const robotCount = Number(responseData.robotcount ?? 0);
        const spriteIndex = Number(responseData.Sprite ?? 0);
        const cur = Number(responseData.current_level_exp ?? 0);
        const need = Number(responseData.next_level_need_exp ?? 0);
        
        // ✅ 提取其他字段
        const alliance = responseData.alliance ?? '';
        ...
        this.setNumericalValue(this.classNode, cls > 0 ? this.getClassName(cls) : '--');
        ...
        const pct = need > 0 ? Math.max(0, Math.min(1, cur / need)) : 0;
        const w = Math.round(this.barMax * pct);
        if (this.expLabel) {
            // 格式化经验值：超过1万显示为"X万"（四舍五入），不超过显示阿拉伯数字
            const formattedCur = this.formatExpValue(cur);
            const formattedNeed = this.formatExpValue(need);
            this.expLabel.string = `${formattedCur}/${formattedNeed}`;
        }
        if (this.expPoint) {
            const ut = this.expPoint.getComponent(UITransform);
        ...
        const playerStatus = responseData.status || (responseData.online ? 'online' : 'offline');
        const isOnline = playerStatus !== 'offline';
        this.updateOnlineStatus(isOnline, playerStatus);
        
        // 更新按钮状态（根据服务器返回的 is_self 和 is_friend）
     ...
     */
    private formatExpValue(value: number): string {
        if (value >= 10000) {
            // 超过1万，转换为"万"单位，四舍五入
            const wanValue = Math.round(value / 10000);
            return `${wanValue}万`;
        } else {
            // 不超过1万，显示阿拉伯数字
            return String(value);
     ...
     */
    private updateOnlineStatus(isOnline: boolean, status?: string): void {
    ...
    private onAddFriend() {
        ...
        const targetCharacterId = this.currentConfig?.characterId;
    ...
    private setNumericalValue(root: Node | null, text: string) {
        if (!root) return;
        const child = root.getChildByName('NumericalValue');
        if (!child) return;
    ...
    private getClassName(cls: number): string {
        switch (cls) {
            case 1: return '格斗家';
            case 2: return '枪械师';
            case 3: return '全能师';
