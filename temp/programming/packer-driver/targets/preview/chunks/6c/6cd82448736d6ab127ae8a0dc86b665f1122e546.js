      ...
      _export("RobotAttributePanel", RobotAttributePanel = (_dec = ccclass('RobotAttributePanel'), _dec2 = property({
        ...
        constructor() {
          ...
          this.onGlobalRobotRefresh = () => {
            ...
            if (!((_this$node = this.node) != null && _this$node.active) || !this._lastShownPetId) return;
        ...
        onLoad() {
          (_crd && robotGameEvents === void 0 ? (_reportPossibleCrUseOfrobotGameEvents({
            ...
            error: Error()
          }), RobotGameEvent) : RobotGameEvent).RobotDataUpdated, this.onGlobalRobotRefresh, this);
          (_crd && robotGameEvents === void 0 ? (_reportPossibleCrUseOfrobotGameEvents({
            error: Error()
          }), robotGameEvents) : robotGameEvents).on((_crd && RobotGameEvent === void 0 ? (_reportPossibleCrUseOfRobotGameEvent({
            error: Error()
          }), RobotGameEvent) : RobotGameEvent).BattleTeamUpdated, this.onGlobalRobotRefresh, this);
        }
        ...
        bindButtonEvents() {
          this.functionButtons.forEach((button, index) => {
            if (button && this.functionPanels[index]) {
              var panelName = this.functionPanels[index].name;
        ...
        hideTarget() {
          var target = this.targetToHide || this.node;
        ...
        switchToPanel(panelName) {
          var targetPanel = this.functionPanels.find(p => p.name === panelName);
        ...
        showPanel(panelName) {
          var panel = this.functionPanels.find(p => p.name === panelName);
        ...
        getAllPanelStates() {
          return _extends({}, this.panelStates);
        }
        /**
         * 获取节点的完整路径（用于调试）
         */
        ...
        showSelectedRobot(petId) {
          this._lastShownPetId = petId ? String(petId) : null; // 隐藏详细日志，只保留错误日志
          // 确保面板显示
        ...
        onDestroy() {
          ...
          (_crd && robotGameEvents === void 0 ? (_reportPossibleCrUseOfrobotGameEvents({
            ...
            error: Error()
          }), RobotGameEvent) : RobotGameEvent).BattleTeamUpdated, this.onGlobalRobotRefresh, this); // 清理事件监听
...
//# sourceMappingURL=6cd82448736d6ab127ae8a0dc86b665f1122e546.js.map