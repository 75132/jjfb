System.register(["cc"], function (_export, _context) {
  ...
  function resolveActiveEventPage(npcUid, events, ctx) {
    ...
    for (const ev of sortedEvents(events)) {
      ...
      if (ctx.isStepComplete(npcUid, ev)) continue;
      if (!ctx.requirementsMet((_ev$server = ev.server) == null ? void 0 : _ev$server.requirements)) continue;
      const ord = (_ev$order = ev.order) != null ? _ev$order : 0;
  ...
  function npcAppearRequirementsMet(appear, initialHidden, requirementsMet) {
    ...
    const mode = (_appear$mode = appear == null ? void 0 : appear.mode) != null ? _appear$mode : initialHidden ? 'conditional' : 'always';
    if (mode !== 'conditional') return mode === 'always';
    const reqs = (_appear$requirements = appear == null ? void 0 : appear.requirements) != null ? _appear$requirements : [];
    if (!reqs.length) return false;
    const matchAny = (appear == null ? void 0 : appear.matchMode) === 'ANY';
