      ...
      _export("FriendPanel", FriendPanel = (_dec = ccclass('FriendPanel'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(ScrollView), _dec7 = property(Node), _dec8 = property(Prefab), _dec9 = property(EditBox), _dec10 = property(Node), _dec11 = property(Node), _dec12 = property(Node), _dec13 = property(Node), _dec14 = property(Node), _dec(_class = (_class2 = class FriendPanel extends Component {
        ...
        onLoad() {
          ...
          if (this.requestListButton) {
            var button = this.requestListButton.getComponent(Button);
          ...
          if (this.friendListButton) {
            var _button = this.friendListButton.getComponent(Button);

            if (_button) {
              _button.node.on(Button.EventType.CLICK, () => this.switchTab('friend'), this);
            } else {
          ...
          if (this.addFriendButton && this.addFriendPanel) {
            var _button2 = this.addFriendButton.getComponent(Button);

            if (_button2) {
              _button2.node.on(Button.EventType.CLICK, () => {
          ...
          if (this.addFriendConfirmButton) {
            var _button3 = this.addFriendConfirmButton.getComponent(Button);

            if (_button3) {
              _button3.node.on(Button.EventType.CLICK, this.onConfirmAddFriend, this);
            } else {
          ...
          if (this.backControlButton) {
            var _button4 = this.backControlButton.getComponent(Button);

            if (_button4) {
              _button4.node.on(Button.EventType.CLICK, this.handleBackControl, this);
            } else {
          ...
          if (this.addFriendResultRoot) {
            var pos = this.addFriendResultRoot.position;
        ...
        buildList(list, mode) {
          ...
          var startX = 240;
          var startY = -30;
          var itemSpacing = 60; // 每个好友项之间的 y 间距

          for (var i = 0; i < list.length; i++) {
            var _f$Sprite, _f$role_name;

            var f = list[i];
            var node = instantiate(this.friendItemPrefab); // 设置位置：第一个 x=240, y=-30，后续每个 y 减 60

            var yPos = startY - i * itemSpacing;
            ...
            var lastIndex = this.content.children.length - 1;

            if (lastIndex >= 0) {
              node.setSiblingIndex(lastIndex);
            }

            var item = node.getComponent(_crd && FriendItem === void 0 ? (_reportPossibleCrUseOfFriendItem({
              error: Error()
        ...
        handleOpenSetPanel(item) {
          // 关闭其他所有 Set 面板
          for (var it of this.currentItems) {
            if (it !== item) {
              it.closeSetPanel();
            }
          }
        ...
        viewFriendInfo(item) {
          ...
          var characterProfile = this.characterProfileNode.getComponent(_crd && CharacterProfile === void 0 ? (_reportPossibleCrUseOfCharacterProfile({
            error: Error()
        ...
        onConfirmAddFriend() {
          ...
          var text = (this.addFriendEditBox.string || '').trim(); // 输入验证：空输入
        ...
        showSearchError(message) {
          ...
          if (this.addFriendResultLabel) {
            var label = this.addFriendResultLabel.getComponent(Label);
        ...
        searchFriendById(friendId) {
          this.ws.request('search_friend', {
            ...
            var f = resp.data.friend;

            if (!this.friendItemPrefab || !this.addFriendResultRoot) {
              return;
            }

            var node = instantiate(this.friendItemPrefab);
            ...
            var lastIndex = this.addFriendResultRoot.children.length - 1;
        ...
        clearAddFriendResult(hideLabel) {
          if (hideLabel === void 0) {
            hideLabel = true;
          }
          ...
          if (this.addFriendResultRoot) {
            ...
            var children = this.addFriendResultRoot.children.slice();
        ...
        handleBackControl() {
          ...
          var isAddFriendPanelOpen = this.addFriendPanel ? this.addFriendPanel.active : false; // 如果有打开的浮层，先关闭它们

          if (hasOpenSetPanel || isAddFriendPanelOpen) {
            // 关闭所有 Set 面板
            for (var _it of this.currentItems) {
              _it.closeSetPanel();
            }
