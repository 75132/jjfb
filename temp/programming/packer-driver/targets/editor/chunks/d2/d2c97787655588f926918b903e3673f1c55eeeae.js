      ...
      _export("MechAttributeTEST", MechAttributeTEST = (_dec = ccclass('MechAttributeTEST'), _dec2 = property({
        ...
        constructor(...args) {
          ...
          this.onRobotInfo = data => {
            try {
              ...
              const normCurrent = this.normPetId(this.currentPetId); // 如果界面已有目标机甲ID，而本次消息没有 petId 或者与当前不一致，则跳过
        ...
        async showSelectedRobot(petId) {
          try {
            ...
            this.currentPetId = wantedId; // 关键修复：先尝试使用缓存数据，解决首次打开不显示的问题

            const cacheManager = (_crd && DataCacheManager === void 0 ? (_reportPossibleCrUseOfDataCacheManager({
              error: Error()
            }), DataCacheManager) : DataCacheManager).getInstance();
            const cachedData = wantedId ? cacheManager.getRobotPetInfoCache(wantedId) : null; // 关键修复：通知 MechEquipment 组件设置 currentPetId
            ...
            if (cachedData && wantedId) {
              ...
              const cRaw = (_cachedData$pet_id = cachedData.pet_id) != null ? _cachedData$pet_id : (_cachedData$data = cachedData.data) == null ? void 0 : _cachedData$data.pet_id;
