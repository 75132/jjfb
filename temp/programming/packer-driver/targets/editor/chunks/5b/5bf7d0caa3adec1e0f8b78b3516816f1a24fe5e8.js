    ...
    execute: function () {
      ...
      _export("MiniGame2", MiniGame2 = (_dec = ccclass('MiniGame2'), _dec2 = property({
            ...
            this.ws.request('minigame2_bet', {
              ...
              if (resp != null && resp.success && resp != null && resp.data) {
                ...
                if (respSelected && respAmt) {
                  const cat = CATEGORIES.find(c => c.key === respSelected);
        ...
        async ensureOptionItemsReady() {
          ...
          const prefab = await this.ensureOptionItemPrefab(); // 清空旧项（content 下理论上为空；但为了多次 mount 也要防御）
            ...
            for (const cat of CATEGORIES) {
              if (prefab) {
                ...
                btnComp.node.on(Button.EventType.CLICK, () => {
                  var _this$_lastPayload, _this$_lastPayload2;

                  // 允许同一期持续下注，只要未开奖且仍在投注时间内
                  if ((_this$_lastPayload = this._lastPayload) != null && _this$_lastPayload.round_drawn) return;
                  if (((_this$_lastPayload2 = this._lastPayload) == null ? void 0 : _this$_lastPayload2.seconds_until_close) !== undefined && this._lastPayload.seconds_until_close <= 0) return;
                ...
                btn.node.on(Button.EventType.CLICK, () => {
                  var _this$_lastPayload3, _this$_lastPayload4;

                  if ((_this$_lastPayload3 = this._lastPayload) != null && _this$_lastPayload3.round_drawn) return;
                  if (((_this$_lastPayload4 = this._lastPayload) == null ? void 0 : _this$_lastPayload4.seconds_until_close) !== undefined && this._lastPayload.seconds_until_close <= 0) return;
                  this.selectedKey = cat.key;
                  if (this.currentSelectionLabel) this.currentSelectionLabel.string = cat.name;
                  this.updateOptionVisual();
                }, this);
        ...
        applyPayload(p) {
          ...
          const canBetBase = !p.round_drawn && ((_p$seconds_until_clos3 = p.seconds_until_close) != null ? _p$seconds_until_clos3 : 0) > 0; // 面板打开时禁止下注交互

          const canBet = canBetBase && !this.investmentReturnHistoryOpen; // 多类目下注：不再强行用“我已下注的类目”覆盖当前选中项，避免你投了多个后 UI 跳来跳去
          // 保持选中类目的“按下状态”高亮
