      ...
      _export("MechEquipment", MechEquipment = (_dec = ccclass('MechEquipment'), _dec2 = property({
        ...
        constructor() {
          ...
          this.onRobotPetInfo = data => {
            try {
              ...
              var success = (data == null ? void 0 : data.success) !== false;
              ...
              var petId = data.pet_id || ((_data$data = data.data) == null ? void 0 : _data$data.pet_id);
              ...
              var equipment = ((_data$data2 = data.data) == null ? void 0 : _data$data2.equipment) || data.equipment || {}; // 缓存一份，面板再次打开时可直接显示
          ...
          this.onUnequipItemResponse = data => {
            try {
              ...
              var success = (_ref = (_data$success = data == null ? void 0 : data.success) != null ? _data$success : data == null || (_data$data3 = data.data) == null ? void 0 : _data$data3.success) != null ? _ref : false;
        ...
        onEnable() {
          ...
          if (this.lastEquipment) {
            ...
            var cachedData = cacheManager.getRobotPetInfoCache(this.currentPetId);

            if (cachedData && cachedData.success !== false) {
              var _cachedData$data;

              // 使用缓存数据更新显示
              var equipment = ((_cachedData$data = cachedData.data) == null ? void 0 : _cachedData$data.equipment) || cachedData.equipment || {};
        ...
        hasEquipment(equipment) {
          ...
          for (var slotName of this.SLOT_NAMES) {
        ...
        initSlotMaps() {
          ...
          var slotNodes = [this.weaponSlot, this.gunSlot, this.dunSlot, this.wingSlot, this.xinpianSlot, this.toukaiSlot, this.jianjiaSlot, this.xiongkaiSlot, this.tuikaiSlot, this.shoukaiSlot];

          var _loop = function _loop() {
            var slotName = _this.SLOT_NAMES[i];
            ...
            _this.slotMap.set(slotName, slotNode); // 查找Label组件（用于显示装备名）


            var labelNode = slotNode.getChildByName('Label');

            if (labelNode) {
              var label = labelNode.getComponent(Label);

              if (label) {
                _this.slotLabelMap.set(slotName, label); // 初始化为空并隐藏
            ...
            var removeNode = slotNode.getChildByName('Remove');

            if (removeNode) {
              var removeButton = removeNode.getComponent(Button);

              if (removeButton) {
                _this.slotRemoveButtonMap.set(slotName, removeButton); // 绑定点击事件


                removeButton.node.on(Button.EventType.CLICK, () => {
                  _this.onRemoveButtonClick(slotName);
                }, _this); // 初始隐藏
        ...
        updateEquipmentDisplay(equipment) {
          for (var slotName of this.SLOT_NAMES) {
            var equippedItem = equipment[slotName];
            var label = this.slotLabelMap.get(slotName);
            var removeButton = this.slotRemoveButtonMap.get(slotName);

            if (equippedItem && equippedItem.item_id) {
              // 有装备，显示装备名和Remove按钮
              var itemName = equippedItem.name || "\u7269\u54C1" + equippedItem.item_id;
        ...
        onRemoveButtonClick(slotName) {
          ...
          var characterId = this.ws.getCharacterId();
          var token = this.ws.getToken();
        ...
        setCurrentPetId(petId) {
          var previousPetId = this.currentPetId;
          ...
          if (petId) {
            ...
            var cachedData = cacheManager.getRobotPetInfoCache(petId);

            if (cachedData && cachedData.success !== false) {
              var _cachedData$data2;

              var equipment = ((_cachedData$data2 = cachedData.data) == null ? void 0 : _cachedData$data2.equipment) || cachedData.equipment || {};
...
//# sourceMappingURL=2c5cd17a154e6464b293f23c2b9b2b69e4153163.js.map