...
@ccclass('GameCommonData')
export class GameCommonData extends Component {
    /** 单例，方便场景内其他脚本访问 */
    public static instance: GameCommonData | null = null;
    ...
    /** 角色名称 */
    @property({ tooltip: '角色名称（由服务器数据同步）' })
    private _roleName: string = '';

    /** 最高等级（默认 60 级封顶） */
    @property({ tooltip: '角色最高等级（默认 60 级）' })
    maxLevel: number = 60;
    ...
    onLoad() {
        ...
        this.wsManager.on('select_character_response', this.onCharacterSelected, this);
        // 监听认证响应，认证成功后再请求数据（关键修复）
        this.wsManager.on('auth_response', this.onAuthResponse, this);
        // 关键修复：监听角色切换事件，清除内部状态
        this.wsManager.on('data_changed', this.onCharacterChanged, this);
    ...
    private triggerLoadingOnDataMissing(reason: string): void {
        ...
        const gameControl = this.node.scene?.getChildByName('Game')?.getComponent('GameControl');
        if (gameControl && typeof (gameControl as any).handleDataIntegrityFailed === 'function') {
            (gameControl as any).handleDataIntegrityFailed(reason);
        }
    }
    ...
    public get needExpForNextLevel(): number {
        if (this.isMaxLevel) {
            return 0;
        }
        const nextLevelTotal = this.getTotalExpForLevel(this._level + 1);
    ...
    public refreshPlayerInfo(callback?: (data: { level: number; totalExp: number; roleName: string }) => void): Promise<{ level: number; totalExp: number; roleName: string }> {
        return new Promise((resolve, reject) => {
            ...
            this.node.once('data_updated', onDataUpdated);
     ...
     */
    public refreshData(dataType: 'player' | 'exp' | 'all' = 'all', callback?: (data: any) => void): void {
        switch (dataType) {
            case 'player':
            case 'all':
    ...
    private requestPlayerInfo(): void {
        ...
        // 构建请求数据
        const requestData: any = { character_id: cid };
    ...
    private onPlayerInfo = (data: any): void => {
        ...
        const totalExp = Number(responseData.total_exp !== undefined ? responseData.total_exp : responseData.exp || 0);
        const roleName = String(responseData.role_name || '');
    ...
    private onAddExpResponse = (data: any): void => {
        ...
        const oldLevel = this._level;
        const level = Number(data.level || this._level);
        const totalExp = Number(data.total_exp || this._totalExp);
        const levelUpCount = data.level_up_count || 0;
     ...
     */
    private onCharacterSelected = (data: any): void => {
        if (data && data.success) {
            // 切换角色后重新请求玩家信息
            this.requestPlayerInfo();
        }
     ...
     */
    private syncFromServer(level: number, totalExp: number, maxLevel?: number): void {
     ...
     */
    public setLevelFromServer(level: number): void {
        this._level = this.clampLevel(level);
    }
     ...
     * 仅更新经验（例如某些接口只返回当前经验）
     */
    public setTotalExpFromServer(totalExp: number): void {
        this._totalExp = Math.max(0, totalExp);
    }
    ...
    private clampLevel(level: number): number {
        const maxL = Math.max(1, Math.min(this.maxLevel, level));
